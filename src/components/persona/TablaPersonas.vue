    ...
    <table class="table">
      ...
      <tbody>
        <tr v-for="persona in personas" :key="persona.id">
          ...
          <td v-if="editando === persona.id">
            <button class="btn btn-success" @click="guardarPersona(persona)">💾 Save</button>
            <button class="btn btn-secondary ml-2" @click="cancelarEdicion(persona)">❌ Cancel</button>
          </td>
          <td v-else>
            <button class="btn btn-info" @click="editarPersona(persona)">✏️ Edit</button>
            <button class="btn btn-danger ml-2" @click="$emit('delete-persona', persona.id)">🗑️ Delete</button>
          </td>
        </tr>
      </tbody>
    </table>
